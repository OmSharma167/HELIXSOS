...
// import DoctorContactSection from './DoctorContactSection';
...
//             <img src={doctor.imageUrl} alt={`${doctor.name}'s profile`} className="rounded-lg w-full h-auto object-cover shadow-lg" />
//           </div>
...
//               {['booking', 'reviews', 'contact', 'chat'].map((tab) => (
//                 <button
//                   key={tab}
...
//               {activeTab === 'contact' && (
//                 <div>
//                   <h2 className="text-2xl font-bold mb-4">Contact {doctor.name}</h2>
//                   <DoctorContactSection doctor={doctor} />
//                 </div>
//               )}
//               {activeTab === 'chat' && (
//                 <div>
//                   <h2 className="text-2xl font-bold mb-4">Chat with {doctor.name}</h2>
...
import React, { useEffect, useState } from 'react';
import { useParams } from 'react-router-dom';
import { Calendar, Clock, MapPin, Award, DollarSign, MessageCircle, Star } from 'lucide-react';
import axios from 'axios';
import BookAppointment from './BookAppointment';
import ReviewRating from './ReviewRating';
import DoctorContactSection from './DoctorContactSection';
      ...
      <div className="bg-white rounded-xl shadow-lg">
        ...
        <div className="flex flex-col lg:flex-row p-4 sm:p-6 gap-6 lg:gap-8">
          ...
          <div className="w-full lg:w-2/3">
            {/* Doctor Info */}
            <div className="mb-6">
              <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 mb-2">{doctor.name}</h1>
              <p className="text-lg sm:text-xl text-blue-600 mb-4">{doctor.specialization}</p>
            ...
            <div className="overflow-x-auto -mx-4 px-4 sm:mx-0 sm:px-0">
              <div className="flex border-b mb-6 min-w-max sm:min-w-0">
                {[
                  { id: 'booking', icon: <Calendar size={18} />, label: 'Book Appointment' },
                  { id: 'reviews', icon: <Star size={18} />, label: 'Reviews' },
                  { id: 'contact', icon: <MapPin size={18} />, label: 'Contact' },
            ...
            <div className="bg-white rounded-xl p-4 sm:p-6 shadow-md">
              {activeTab === 'booking' && (
                <div>
                  <h2 className="text-xl sm:text-2xl font-bold mb-4">Book an Appointment</h2>
                  <BookAppointment doctorId={doctorId} />
                </div>
              )}
              {activeTab === 'reviews' && (
                <div>
                  <h2 className="text-xl sm:text-2xl font-bold mb-4">Reviews and Ratings</h2>
                  <ReviewRating doctorId={doctorId} />
                </div>
              )}
              {activeTab === 'contact' && (
                <div>
                  <h2 className="text-xl sm:text-2xl font-bold mb-4">Contact {doctor.name}</h2>
                  <DoctorContactSection doctor={doctor} />
                </div>
              )}
              {activeTab === 'chat' && (
                <div>
                  <h2 className="text-xl sm:text-2xl font-bold mb-4">Chat with {doctor.name}</h2>
                  <Chat doctorId={doctorId} userId={userId} />
                </div>
              )}
            </div>
